...
description: >
  ...
  analiz eder. Çıktı, WordPress REST API (/wp-json/wp/v2/pages) ile doğrudan import edilebilir 
  veya Gutenberg Kod Editörüne yapıştırılabilir formattadır — iç içe konteynerler, grup blokları 
  ve gelişmiş düzen yapılarıyla estetik, katmanlı tasarımlar üretir. Anahtar kelimeler: 
  "WordPress JSON", "WP JSON import", "Starter Templates", "Spectra blok", "iç içe konteyner", 
  "WordPress sayfa tasarımı", "referans URL", "tasarım kopyala", "JSON çıktı", "blok editörü", 
  "Gutenberg JSON", "sayfa şablonu", "bu siteye benzer", "bu URL gibi yap".
---
...
**Bu skill YALNIZCA WordPress tasarımları için kullanılır.**
...
2.  [WordPress Uyumluluk Gereksinimleri](#wordpress-uyumluluk)
3.  [Tasarım Felsefesi](#tasarım-felsefesi)
4.  [Adım Adım İş Akışı](#adım-adım-iş-akışı)
5.  [JSON Çıktı Formatı — WordPress REST API Uyumlu](#json-çıktı-formatı)
6.  [İç İçe Konteyner Tasarım Sistemi](#iç-içe-konteyner-sistemi)
7.  [WordPress Blok Türleri Referansı](#blok-türleri-referansı)
8.  [Tasarım Sadakati Rehberi](#tasarım-sadakati-rehberi)
9.  [İçerik Yerleştirme Protokolü](#içerik-yerleştirme-protokolü)
10. [Çıktı Yapısı Şablonu](#çıktı-yapısı-şablonu)
11. [Kalite Kontrol Listesi](#kalite-kontrol-listesi)
12. [Sık Yapılan Hatalar ve Düzeltmeleri](#sık-yapılan-hatalar)
13. [Son Çıktı Protokolü](#son-çıktı-protokolü)

---

## Bu Skill Ne Zaman Tetiklenir

- Kullanıcı bir referans web sitesi URL'si paylaşır ve benzer tasarımda WordPress sayfası ister
...
ritmini ve estetiğini kullanarak YENİ bir WordPress sayfası üretir.

---

## WordPress Uyumluluk Gereksinimleri {#wordpress-uyumluluk}

Bu skill'in ürettiği JSON dosyası **üç farklı yöntemle** WordPress'e aktarılabilir olmalıdır:

### Yöntem 1: WordPress REST API (Birincil)

JSON çıktısı, `POST /wp-json/wp/v2/pages` endpoint'ine doğrudan gönderilebilir formatta olmalıdır.
...
**KRİTİK:** REST API'de `content` alanı doğrudan **string** olmalıdır, `content.raw` olarak DEĞİL. REST API `content` alanını string olarak kabul eder. Eğer `content.raw` formatı da desteklenmek isteniyorsa, JSON dosyasına her iki versiyonu da ekle:

```json
{
  "title": "Sayfa Başlığı",
  "status": "draft",
  "content": "<!-- blok içeriği string olarak -->",
  "template": "",
  "meta": {}
}
```

### Yöntem 2: Gutenberg Kod Editörüne Yapıştırma

`content` alanındaki blok serileştirme HTML'i, WordPress yazı editöründe "Kod Editörü" moduna geçilip doğrudan yapıştırılabilir olmalıdır.

### Yöntem 3: Starter Templates / Spectra JSON Import

Starter Templates veya Spectra eklentisi kullanılıyorsa uyumlu format:
```json
{
  "title": "Sayfa Başlığı",
  "status": "draft",
...
### WordPress Blok Doğrulama Kuralları

WordPress, blok HTML'ini parse ederken belirli kuralları kontrol eder. Bunlara uymayan bloklar "geçersiz blok" olarak işaretlenir ve editörde bozuk görünür.

**ZORUNLU kurallar:**

1. **Her blok yorum etiketi JSON attribute'ları içermelidir** (eğer attribute varsa)
   ```html
   <!-- wp:group {"align":"full","style":{"color":{"background":"#f8f9fa"}}} -->
   ```

2. **HTML etiketi ile blok yorumundaki attribute'lar tutarlı olmalıdır**
   - Yorum: `{"style":{"color":{"background":"#f8f9fa"}}}`
   - HTML: `style="background-color:#f8f9fa"`
   - Bu ikisi aynı değeri yansıtmalıdır

3. **CSS class'ları WordPress kurallarına uymalıdır**
   - Arka plan varsa: `has-background` class'ı zorunlu
   - Metin rengi varsa: `has-text-color` class'ı zorunlu
   - Hizalama varsa: `alignfull`, `alignwide` class'ları zorunlu
   - Blok tipi: `wp-block-group`, `wp-block-columns` vb. zorunlu

4. **İç içe bloklar doğru sırada kapatılmalıdır**
   ```html
   <!-- wp:group -->        ← Açıldı
   <div class="wp-block-group">
     <!-- wp:heading -->     ← Açıldı
     <h2>Başlık</h2>
     <!-- /wp:heading -->    ← Kapatıldı (FIFO - ilk açılan son kapanır DEĞİL, LIFO)
   </div>
   <!-- /wp:group -->        ← Kapatıldı
   ```

5. **Bloklar arası boş satır (`\n\n`) olmalıdır** — WordPress bunu blok ayırıcı olarak kullanır

6. **Self-closing bloklar** (spacer, separator) farklı formattadır:
   ```html
...
7. **Buttons bloğu özel yapıya sahiptir:**
   ```html
   <!-- wp:buttons {"layout":{"type":"flex","justifyContent":"center"}} -->
   <div class="wp-block-buttons">
     <!-- wp:button {"style":{"border":{"radius":"8px"},"color":{"background":"#2563eb","text":"#ffffff"}}} -->
     <div class="wp-block-button"><a class="wp-block-button__link has-text-color has-background wp-element-button" style="border-radius:8px;color:#ffffff;background-color:#2563eb">Buton Metni</a></div>
     <!-- /wp:button -->
   </div>
   <!-- /wp:buttons -->
   ```
...
## JSON Çıktı Formatı — WordPress REST API Uyumlu {#json-çıktı-formatı}

Çıktı, WordPress REST API'ye (`POST /wp-json/wp/v2/pages`) doğrudan gönderilebilecek
formatta JSON dosyasıdır.

### WordPress REST API JSON Yapısı
...
**KRİTİK:** `content` alanı doğrudan **string** tipindedir — nested object (`content.raw`) DEĞİLDİR. WordPress REST API v2 `content` alanını string olarak kabul eder.
...
1. **`content` değeri tek bir string olmalıdır** — JSON escape ile düzgün yazılmalı
...
5. **Bloklar arasında `\n\n` ile boş satır bırakılır** (WordPress blok ayırıcısı)
6. **Blok yorum etiketindeki JSON ile HTML'deki inline style tutarlı olmalıdır**
7. **WordPress class kurallarına uyulmalıdır** (has-background, has-text-color, alignfull vb.)
...
```html
<!-- wp:columns {"isStackedOnMobile":true,"style":{"spacing":{"blockGap":{"left":"32px"}}}} -->
...
**WordPress Tema Çakışma Önleme:**
- `wp:html` bloğu içinde class isimleri benzersiz olmalı (prefix kullan: `wps-card`, `wps-grid` vb.)
- `!important` kullanımını minimumda tut ama gerektiğinde kullan
- Font tanımları WordPress temasıyla çakışabilir — `wp:html` içinde kendi font stack'ini belirt

---

## Kalite Kontrol Listesi

### WordPress REST API Uyumluluk Kontrolleri (YENİ — KRİTİK)

- [ ] `content` alanı string tipinde (REST API uyumu)
- [ ] JSON dosyası `json.dumps()` ile üretilmiş (escape hataları yok)
- [ ] JSON dosyası valid — `python -m json.tool` ile doğrulanmış
- [ ] Blok yorum etiketlerindeki JSON attribute'ları valid JSON
- [ ] HTML class'ları WordPress kurallarına uygun (has-background, alignfull vb.)
- [ ] Blok yorum JSON'u ile inline style değerleri tutarlı
- [ ] Bloklar arası `\n\n` ayırıcılar mevcut
- [ ] Buttons bloğu doğru yapıda (wp:buttons > wp:button > a.wp-block-button__link)
- [ ] Self-closing bloklar (spacer, separator) doğru formatta

### Yapısal Kontroller

- [ ] Her blok açma ve kapama yorumlarıyla sarılı
- [ ] İç içe bloklar doğru hiyerarşide (LIFO kapanış sırası)
- [ ] Tüm blok attribute'ları valid JSON nesneleri
- [ ] Escape edilmemiş çift tırnak YOK (content string içinde)
...
| 2 | `content` alanı nested object (`content.raw`) | REST API için `content` doğrudan string olmalı |
| 3 | Düz blok dizilimi (iç içe konteyner yok) | Her bölümü min. 2 katman group ile sar |
| 4 | Tüm bölümlerde aynı arka plan | Dönüşümlü arka plan renkleri kullan |
| 5 | Blok kapama yorumu eksik | Her `<!-- wp:x -->` için `<!-- /wp:x -->` olmalı |
| 6 | Attribute'larda tek tırnak | JSON'da sadece çift tırnak kullan |
| 7 | `style` attribute'unda object yerine string | `"style":{"color":{"background":"#fff"}}` doğru format |
| 8 | Padding'siz konteynerler | Her katmanda padding tanımla |
| 9 | Monoton kart tasarımı | Bölümlere göre farklı kart varyasyonları kullan |
| 10 | Font bilgisi eksik | Kullanıcıya hangi Google Font'ların gerektiğini bildir |
| 11 | `alignfull` eksik dış gruplarda | Tam genişlik bölümler için `"align":"full"` ekle |
| 12 | Responsive düşünülmemiş | `isStackedOnMobile` ve mobil padding değerlerini ekle |
| 13 | `has-background` class'ı eksik | Arka plan rengi olan her blokta bu class zorunlu |
| 14 | Yorum JSON'u ile inline style uyumsuz | İkisi aynı değerleri yansıtmalı |
| 15 | Buttons bloğu yanlış yapıda | wp:buttons > wp:button > a.wp-block-button__link |
| 16 | Bloklar arası `\n\n` eksik | Her blok arasında boş satır (blok ayırıcı) olmalı |
...
4. **`content` string'ini oluştur** — blok serileştirme formatında
...
7. **Dosyayı `/mnt/user-data/outputs/wordpress-page.json` olarak kaydet**
...
content_blocks = """<!-- wp:group {"align":"full","style":{"color":{"background":"#f8f9fa"},"spacing":{"padding":{"top":"80px","bottom":"80px","left":"24px","right":"24px"}}},"layout":{"type":"constrained"}} -->
<div class="wp-block-group alignfull has-background" style="background-color:#f8f9fa;padding-top:80px;padding-bottom:80px;padding-left:24px;padding-right:24px">

<!-- wp:heading {"level":2,"style":{"typography":{"fontSize":"36px","fontWeight":"700"},"color":{"text":"#1a1a2e"}}} -->
<h2 class="wp-block-heading has-text-color" style="color:#1a1a2e;font-size:36px;font-weight:700">Başlık</h2>
<!-- /wp:heading -->

</div>
<!-- /wp:group -->"""

# WordPress REST API uyumlu JSON yapısı
page_data = {
    "title": "Sayfa Başlığı",
    "status": "draft",
    "content": content_blocks,  # Doğrudan string — REST API uyumlu
    "template": "",
    "meta": {}
}
...
# Doğrulama
with open('/mnt/user-data/outputs/wordpress-page.json', 'r') as f:
...
3. **WordPress'e aktarım yöntemleri** (üç seçenek):

   **Yöntem A — REST API ile (önerilen):**
   ```
   POST /wp-json/wp/v2/pages
   Header: Content-Type: application/json
   Header: Authorization: Bearer YOUR_TOKEN
   Body: JSON dosyasının içeriği
   ```

   **Yöntem B — Gutenberg Kod Editörü ile:**
   ```
   1. WordPress'te yeni sayfa oluştur
   2. Sağ üst menüden "Kod Editörü"ne geç
   3. JSON dosyasındaki "content" alanının değerini yapıştır
   4. "Görsel Editör"e geri dön — bloklar oluşmuş olacak
   ```

   **Yöntem C — Starter Templates eklentisi ile:**
   ```
   Eğer Starter Templates/Spectra yüklüyse, JSON import özelliğini kullan
   (Bu durumda content.raw formatı gerekebilir — eklenti dökümantasyonuna bak)
   ```
